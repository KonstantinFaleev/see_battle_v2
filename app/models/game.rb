class Game < ActiveRecord::Base
  ...
  class Game < ActiveRecord::Base
    ...
    # If the move was valid then sets the play_status property
    def do_move(x, y)

      #if !does_context_allow_move(player_a) then return end
    ...
    def set_play_status(player)
      if game_over?
        ...
        if self.winner == self.player_a then
          self.play_status = "#You have won. Congratulations!"
        else
